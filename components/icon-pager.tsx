...
const ICON_ROUTES =
  SITE_SECTIONS.find((section) => section.label === "Icons")?.children ?? [];

export function IconPager() {
  const pathname = usePathname();

  const index = ICON_ROUTES.findIndex((item) => item.href === pathname);
  if (index === -1) return null;

  const prev = ICON_ROUTES[index - 1];
  const next = ICON_ROUTES[index + 1];
...
function PagerButton({
  ...
  return (
    <Link
      aria-label={
        direction === "prev"
          ? `Previous icon page: ${target.label}`
          : `Next icon page: ${target.label}`
      }
