...
const CliBlock = ({ icons, staticIconName, className }: CliBlockProps) => {
    ...
    startTransition(async () => {
      ...
      try {
        await navigator.clipboard.writeText(
          `${getPackageManagerPrefix(packageName)} shadcn@latest add @iconiq/${iconName}`
        );
  ...
  return (
    <div
      className={cn("relative mt-[40px] w-full max-w-[642px] px-0", className)}
    >
      <Tabs
        className="w-full"
...
const CliBlockUrl = ({
  ...
  const [packageName, setPackageName] = useState(
    () => Object.values(PACKAGE_MANAGER)[0]
  );
  ...
  const handleCopyToClipboard = () => {
    startTransition(async () => {
      const iconName =
        staticIconName || currentIconName.current || icons?.[0]?.name || "";
      const url = `${SITE.URL}/r/${iconName}.json`;

      try {
        await navigator.clipboard.writeText(
          `${getPackageManagerPrefix(packageName)} shadcn@latest add "${url}"`
        );
        ...
        {Object.values(PACKAGE_MANAGER).map((pm) => (
          ...
          >
            <BaseScrollArea.Root className="relative w-full overflow-hidden">
              ...
              >
                <span className="sr-only">
                  {getPackageManagerPrefix(pm)} shadcn@latest add &quot;
                  {SITE.URL}/r/{staticIconName || currentIconName.current}.json
                  &quot;
                </span>
                <span
                  aria-hidden="true"
