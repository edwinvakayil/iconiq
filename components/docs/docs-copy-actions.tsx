...
function getChatGptPrompt(pageUrl: string, title: string) {
  return encodeURIComponent(
    `I'm looking at the Iconiq documentation page "${title}" (${pageUrl}). Help me understand the page, its philosophy, and how it fits into the library.`
  );
}

export function DocsCopyActions({
  pageContent,
  pageUrl,
  sourceHref,
  title,
  ...
  const menuItems = useMemo(
    () => [
      ...(sourceHref
        ...
        : []),
      {
        href: `https://chatgpt.com/?q=${getChatGptPrompt(absolutePageUrl, title)}`,
        label: "Ask ChatGPT",
      },
    ],
    [absolutePageUrl, sourceHref, title]
  );
